...
function backtestData(candles: Candle[], index: number): AdvancedMarketData {
  ...
  return {
    ...
    microstructure: { spreadPct: null, orderBookImbalance: null, buyNotional: 0, sellNotional: 0, deltaNotional: 0, tradeCount: 0, liquidationBuyNotional: 0, liquidationSellNotional: 0, liquidationNetNotional: 0 },
    derivatives: { basisPct: null },
    sourceHealth: {},
    warnings: [],
  };
}
...
export function runBacktest(candles: Candle[], lookahead = 12): BacktestResult {
  const clean = candles.filter(c => Number.isFinite(c.openTime) && Number.isFinite(c.close) && c.close > 0 && c.high >= c.low).sort((a, b) => a.openTime - b.openTime);
  ...
  for (let i = 60; i < clean.length - horizon; i += 1) {
    ...
    trades.push({ index: i, entryTime: clean[i].openTime, entry: clean[i].close, exit: future.at(-1) ?? null, side: signal.signal, outcome: evaluation.outcome, returnPct: evaluation.realizedReturnPct });
  }
  ...
  return { trades, evaluated: closed.length, wins, losses, winRate: closed.length ? wins / closed.length : null, netReturnPct, averageReturnPct: closed.length ? netReturnPct / closed.length : null, maxDrawdownPct, profitFactor: grossLoss ? grossProfit / grossLoss : grossProfit ? Infinity : null, skipped };
}
