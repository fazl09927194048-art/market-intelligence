...
export type AdvancedMarketData = {
  ...
  futures: {
    ...
    liquidations: Array<{ symbol: string; side: 'BUY' | 'SELL'; price: number; quantity: number; time: number }>;
  };
...
function candle(row: unknown): Candle | null {
  ...
  if (values.slice(0, 7).some(v => v === null)) return null;
...
export async function getAdvancedMarketData(rawSymbol = 'BTCUSDT', rawInterval = '1m', rawLimit = DEFAULT_LIMIT): Promise<AdvancedMarketData> {
  ...
  const limit = Math.min(MAX_LIMIT, Math.max(1, Number.isFinite(Number(rawLimit)) ? Number(rawLimit) : DEFAULT_LIMIT));
  ...
  const [spotPrice, spotCandles, orderBook, trades, futuresPrice, futuresCandles, funding, oi, liquidations] = await Promise.all([
    ...
    safe('Binance Spot Trades', async () => {
      ...
      return (rows ?? []).map((r: Record<string, unknown>) => ({
        ...
        r.id !== null && r.price !== null && r.price > 0 && r.quantity !== null && r.quantity >= 0 && r.time !== null) as Trade[];
    ...
    safe('Binance Futures Liquidations', async () => {
      ...
      return (rows ?? []).map((r: Record<string, unknown>) => ({
        symbol: String(r.symbol ?? symbol), side: r.side === 'SELL' ? 'SELL' : 'BUY', price: n(r.price), quantity: n(r.origQty), time: n(r.time),
      })).filter((r: { price: number | null; quantity: number | null; time: number | null }) => r.price !== null && r.price > 0 && r.quantity !== null && r.quantity >= 0 && r.time !== null);
    }, [], warnings, sourceHealth),
  ]);
