...
export function analyzeChartPatterns(input:Candle[]):ChartPatternSnapshot{
  ...
  const confidence=Math.min(100,Math.round((c.length>=20?50:25)+(pattern!=='NONE'?25:0)+(breakout!=='NONE'?15:0)+(momentum!=='NEUTRAL'?10:0))); if(pattern!=='NONE'&&c.length<20)warnings.push('Pattern detected on a short candle sample; treat as lower-confidence context.');
