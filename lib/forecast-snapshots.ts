...
export async function evaluateDueForecasts(limit=50):Promise<ForecastEvaluation[]>{
 const due=await getDueForecastSnapshots(limit);const results:ForecastEvaluation[]=[];
 for(const s of due){try{const data=await getAdvancedMarketData(s.symbol,s.interval,50);const price=data.futures.price??data.spot.price;if(!Number.isFinite(price)||price<=0)continue;const result=evaluateForecast({...s,endPrice:price});results.push(result);}catch{/* individual evaluation must not block the batch */}}
 return results;
}
