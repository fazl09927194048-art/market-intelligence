...
export function evaluateForecast(input:{id?:string;symbol:string;forecastAt:string;bias:'BULLISH'|'BEARISH'|'NEUTRAL'|'UNAVAILABLE';confidence:number;startPrice:number;endPrice:number;expectedLow:number|null;expectedHigh:number|null;horizonMs:number;now?:string}):ForecastEvaluation{
