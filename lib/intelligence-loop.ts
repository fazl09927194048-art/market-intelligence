...
function parse(xml: string, category: string, source: string): Story[] {
  return [...xml.matchAll(/<item[\s\S]*?<\/item>/gi)].slice(0, 30).map(block => {
    ...
    const published = get('pubDate') || get('published') || get('updated');
...
async function loadNews(): Promise<NewsItem[]> {
  const results = await Promise.allSettled(FEEDS.map(async feed => {
    const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), NEWS_TIMEOUT_MS);
    try {
...
function summarizeNewsImpact(news: NewsItem[], events: ReturnType<typeof detectEvents>) {
  const breaking = events.filter(e => e.impact === 'BREAKING'); const high = events.filter(e => e.impact === 'HIGH');
  ...
  return { level: breaking.length ? 'BREAKING' : high.length ? 'HIGH' : news.length ? 'MEDIUM' : 'NONE', breakingCount: breaking.length, highImpactCount: high.length, bullishHighImpact: bullish, bearishHighImpact: bearish, latestPublishedAt: newest?.publishedAt ?? null, latestAgeMs: newest?.freshnessMs ?? null, sourceCount: new Set(news.map(n => n.source)).size, headlines: breaking.slice(0, 5).map(e => ({ title: e.title, source: e.source, assets: e.assets, sentiment: e.sentiment, url: e.url, publishedAt: e.publishedAt })) };
}
...
export async function runIntelligenceCycle(symbol = 'BTCUSDT', interval = '15m', chartInput?: Partial<ChartContext> | null) {
  ...
  const technical = analyzeTechnical(candles, advanced.spot.orderBook.bids, advanced.spot.orderBook.asks); const chartPatterns = analyzeChartPatterns(candles); const signal = buildSignal(advanced, technical); const forecast = buildForecast(advanced, technical); const risk = assessRisk(advanced, technical, signal);
  const asset = safeSymbol.replace(/USDT$/i, '').toUpperCase(); const assetNews = news.filter(item => item.assets.includes(asset) || item.assets.length === 0); const events = detectEvents(assetNews); const newsImpact = summarizeNewsImpact(assetNews, events); const eventReaction = assessEventReaction(assetNews, risk); const analysts = await runAnalystBrain(advanced, technical, assetNews); const consensus = synthesizeOpinions(analysts); const cycleId = `${safeSymbol}-${Date.now()}`;
  ...
  if (chart?.quality === 'partial') warnings.push('Browser chart context is partial: page price was observed but could not be fully verified.'); if (chart?.quality === 'verified' && chart.ageMs > 120000) warnings.push('Browser chart context is older than 2 minutes.'); if (multiTimeFrame.conflict) warnings.push('Timeframe conflict detected: directional structure disagrees across sampled intervals.');
  const startPrice = advanced.futures.price ?? advanced.spot.price; const forecastAt = new Date().toISOString();
  if (startPrice !== null && Number.isFinite(startPrice) && startPrice > 0 && forecast.bias !== 'UNAVAILABLE') { const forecastBucket = Math.floor(Date.now() / 60_000); const snapshotId = `${safeSymbol}:${safeInterval}:${forecastBucket}`; void recordForecastSnapshot({ id: snapshotId, symbol: safeSymbol, interval: safeInterval, forecastAt, bias: forecast.bias, confidence: forecast.confidence, startPrice, expectedLow: forecast.expectedLow, expectedHigh: forecast.expectedHigh, horizonMs: horizonMs(safeInterval), cycleId }).catch(() => undefined); }
  return { cycleId, generatedAt: forecastAt, symbol: safeSymbol, interval: safeInterval, chartContext: chart, chartPatterns, multiTimeframe, risk, eventReaction, market, marketData: advanced, technical, signal, forecast, analysts, consensus, news: assetNews.slice(0, 30), events: events.slice(0, 10), newsImpact, dataValid: market.markets.length > 0 && candles.length >= 20 && technical.confidence >= 50, sourceHealth: { ...market.sourceHealth, ...advanced.sourceHealth }, warnings };
}
