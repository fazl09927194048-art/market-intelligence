...
function parse(xml:string,category:string,source:string):Story[] {
  return [...xml.matchAll(/<item[\s\S]*?<\/item>/gi)].slice(0,20).map(block=>{
    const item=block[0]; const get=(tag:string)=>clean(item.match(new RegExp(`<${tag}[^>]*>([\\s\\S]*?)</${tag}>`,'i'))?.[1]??'');
    const published=get('pubDate'), parsedDate=published?Date.parse(published):NaN;
...
async function loadNews():Promise<NewsItem[]> {
  const results=await Promise.allSettled(FEEDS.map(async feed=>{ const response=await fetch(feed.url,{headers:{'User-Agent':'MarketIntelligence/1.0'},cache:'no-store'}); if(!response.ok)throw new Error(`${feed.source}:${response.status}`); return parse(await response.text(),feed.category,feed.source); }));
...
export async function runIntelligenceCycle(symbol='BTCUSDT',interval='15m',chartInput?:Partial<ChartContext>|null) {
  const safeSymbol=String(symbol).toUpperCase().replace(/[^A-Z0-9]/g,'')||'BTCUSDT'; const safeInterval=normalizeInterval(interval);
  ...
  const asset=safeSymbol.replace(/USDT$/i,'').toUpperCase(); const assetNews=news.filter(item=>item.assets.includes(asset)||item.assets.length===0);
  const events=detectEvents(assetNews); const newsImpact=summarizeNewsImpact(assetNews,events); const analysts=await runAnalystBrain(advanced,technical,assetNews); const consensus=synthesizeOpinions(analysts);
