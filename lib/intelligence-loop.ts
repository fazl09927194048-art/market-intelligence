...
function summarizeNewsImpact(news:NewsItem[], events:ReturnType<typeof detectEvents>) {
  ...
  return {
    level: breaking.length?'BREAKING':high.length?'HIGH':news.length?'MEDIUM':'NONE',
    breakingCount:breaking.length,
    highImpactCount:high.length,
    bullishHighImpact:bullish,
    bearishHighImpact:bearish,
    latestPublishedAt:newest?.publishedAt??null,
    latestAgeMs:newest?.freshnessMs??null,
    headlines:breaking.slice(0,5).map(e=>({title:e.title,source:e.source,assets:e.assets,sentiment:e.sentiment,url:e.url,publishedAt:e.publishedAt})),
  };
}
...
export async function runIntelligenceCycle(symbol='BTCUSDT',interval='15m') {
  ...
  return {
    ...
    symbol, interval, market, marketData:advanced, technical, signal, forecast, analysts, consensus,
    news:assetNews.slice(0,20), events:events.slice(0,10), newsImpact,
    dataValid:market.markets.length>0&&candles.length>=20&&technical.confidence>=50,
    sourceHealth:advanced.sourceHealth, warnings,
  };
}
