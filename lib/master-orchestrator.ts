...
export function orchestrateAnalysts(symbol: string, interval: string, opinions: AnalystOpinion[], consensus: Consensus): OrchestrationResult {
  const expectedIds = new Set(ANALYSTS.map(analyst => analyst.id));
  const uniqueIds = new Set(opinions.map(opinion => opinion.id));
  const unknownIds = opinions.filter(opinion => !expectedIds.has(opinion.id)).map(opinion => opinion.id);
  const usable = opinions.filter(x => x.id !== 'critic' && x.id !== 'verifier' && expectedIds.has(x.id));
  const contradictions = opinions
    .flatMap(x => x.conflicts.map(conflict => `${x.id}: ${conflict}`))
    .slice(0, 12);
  const coverage = uniqueIds.size === ANALYSTS.length && unknownIds.length === 0 && [...expectedIds].every(id => uniqueIds.has(id));
  const validScores = opinions.every(x => Number.isFinite(x.score) && Number.isFinite(x.confidence));
  const validConsensus = Number.isFinite(consensus.confidence) && Number.isFinite(consensus.score) && Number.isFinite(consensus.agreement);
  const confidence = clamp(Number.isFinite(consensus.confidence) ? consensus.confidence : 0, 0, 96);
  const blocked = !coverage || !validScores || !validConsensus || usable.length < Math.max(10, Math.floor(ANALYSTS.length * 0.6));
  const validationReasons: string[] = [];
  if (!coverage) validationReasons.push(`Specialist coverage invalid: ${uniqueIds.size}/${ANALYSTS.length} unique expected roles returned.`);
  if (unknownIds.length) validationReasons.push(`Unknown specialist IDs detected: ${[...new Set(unknownIds)].slice(0, 5).join(', ')}.`);
  if (!validScores) validationReasons.push('One or more specialist outputs contain non-finite scores or confidence.');
  if (!validConsensus) validationReasons.push('Consensus contains non-finite confidence, score, or agreement.');
  ...
  return {
    stages: [
      ...
      {name: 'SPECIALIST_ANALYSIS', status: coverage ? 'complete' : 'blocked', detail: `${uniqueIds.size}/${ANALYSTS.length} unique specialist roles returned.`},
      {name: 'CROSS_REVIEW', status: 'complete', detail: `${contradictions.length} explicit conflict records and ${Number.isFinite(consensus.agreement) ? consensus.agreement : 0}% directional agreement.`},
    ...
    routing: {totalSpecialists: ANALYSTS.length, activeSpecialists: uniqueIds.size, specialistIds: [...uniqueIds]},
    crossReview: {agreement: clamp(Number.isFinite(consensus.agreement) ? consensus.agreement : 0, 0, 100), dissentCount: consensus.dissent?.length ?? 0, contradictions},
