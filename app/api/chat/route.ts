...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const key = `${MODEL}|${extensionEnabled ? extensionContext : ''}|${context}|${JSON.stringify(chartData)}|${JSON.stringify(chartVision)}|${JSON.stringify(centralIntelligence?.signal ?? null)}|${imageData.slice(0, 64)}|${message}`.slice(0, 50000);
