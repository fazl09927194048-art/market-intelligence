...
async function extractChartVision(apiKey:string, model:string, imageData:string): Promise<import('@/lib/signal').ChartVisionContext|null> {
  const prompt='You are a professional chart-vision engine. Inspect the uploaded trading chart at maximum visual detail. Identify visible symbol/timeframe, candlestick structure, trend/regime, swing highs/lows, support/resistance from the price scale, breakout or fakeout, liquidity sweeps, consolidation/range, volume/order-flow overlays, VWAP/EMA/SMA/RSI/MACD/Bollinger or other visible indicators, divergences, chart patterns, annotations and invalidation clues. Separate READABLE facts from uncertain observations. Never invent a price or indicator value that cannot be read. Return ONLY valid JSON with keys direction (BULLISH|BEARISH|NEUTRAL|UNKNOWN), confidence (0-100), trend, timeframe, price (number|null), support (number[]), resistance (number[]), patterns (string[]), indicators (string[]), structure (string[]), divergence (string[]), liquidity (string[]), invalidation (string|null), evidence (string[]), uncertainties (string[]).');
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({ model, input: imageData ? [{ role: 'user', content: [{ type: 'input_text', text: prompt }, { type: 'input_image', image_url: imageData }] }] : prompt, max_output_tokens: 2200 }),
        cache: 'no-store',
...
export async function POST(request: NextRequest) {
  ...
  if (tooLarge(request, 6_000_000)) return jsonError('Request payload is too large. Compress the chart image and retry.', 413);
  try {
