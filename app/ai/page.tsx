...
export default function AIPage(){
 ...
 const prepareImage=(file:File)=>new Promise<string>((resolve,reject)=>{if(file.size>12*1024*1024)return reject(new Error('Image is too large. Maximum 12MB.'));const img=new Image(),reader=new FileReader();reader.onload=()=>{img.onload=()=>{const max=2400,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.width*scale));canvas.height=Math.max(1,Math.round(img.height*scale));const ctx=canvas.getContext('2d');if(!ctx)return reject(new Error('Image processing unavailable'));ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL('image/jpeg',.9))};img.onerror=()=>reject(new Error('Invalid image'));img.src=String(reader.result)};reader.onerror=()=>reject(new Error('Could not read image'));reader.readAsDataURL(file)});
 const handleImageFile=async(e:any)=>{const file=e?.currentTarget?.files?.[0];if(e?.currentTarget)e.currentTarget.value='';if(!file)return;try{const prepared=await prepareImage(file);setImageData(prepared);setImageName(file.name);setImageResult(null);setError('')}catch(err){setError(err instanceof Error?err.message:'Image error')}};
  const scanImage=useCallback(async()=>{if(!imageData||imageLoading)return;setImageLoading(true);setError('');setImageResult(null);try{const r=await fetch('/api/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({message:`IMAGE DEEP SCAN — ${symbol} ${interval}. Treat the uploaded chart as the primary visual evidence. Inspect candlesticks/price action, visible timeframe, trend and market structure, swing highs/lows, support/resistance, breakouts/fakeouts, liquidity sweeps, gaps, volume/order-flow overlays, indicators, divergence, chart annotations and visible price scale. Then cross-check every readable visual fact against live market data and the central intelligence engine, 33 specialist analysts, multi-timeframe alignment, derivatives, order flow, scenarios, risk and invalidation. Produce a complete actionable analysis specifically for THIS IMAGE. Never invent unreadable numbers; explicitly mark uncertain/hidden values. A signal is allowed only when image evidence and live evidence are sufficiently aligned; otherwise return NO TRADE.`,symbol,interval,context:JSON.stringify(context),toolContext:{imageDeepScan:true,symbol,interval},imageData}),cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d?.error||'Image scan failed');setImageResult(d)}catch(e){setError(e instanceof Error?e.message:'Image scan failed')}finally{setImageLoading(false)}},[imageData,imageLoading,symbol,interval]);
 ...
 return <main className="aiPage" dir={fa?'rtl':'ltr'}>
  ...
  <section className="aiHero">
   ...
   <div className="aiControls">
    ...
    <div className="aiControls">\n     <select value={symbol} onChange={e=>setSymbol(e.currentTarget.value)}>{SYMBOLS.map(x=><option key={x}>{x}</option>)}</select>\n     <select value={interval} onChange={e=>setIntervalValue(e.currentTarget.value)}>{INTERVALS.map(x=><option key={x}>{x}</option>)}</select>\n     <button className="deep" onClick={()=>void refresh()} disabled={loading||imageLoading}>{loading?"SCANNING…":fa?"اسکن کامل":"FULL SCAN"}</button>\n     <button type="button" className="imageScanBtn" title="Upload trading chart image" onClick={()=>imageInputRef.current?.click()}>🖼 {imageName?(fa?"تغییر تصویر":"CHANGE IMAGE"):(fa?"افزودن تصویر":"ADD IMAGE")}</button>\n     <input ref={imageInputRef} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={handleImageFile}/>\n     {imageData&&<button type="button" className="imageDeep" onClick={()=>void scanImage()} disabled={imageLoading}>{imageLoading?(fa?"در حال اسکن تصویر…":"SCANNING IMAGE…"):(fa?"اسکن عمیق تصویر":"DEEP SCAN IMAGE")}</button>}\n    </div>
   </div>
  </section>
