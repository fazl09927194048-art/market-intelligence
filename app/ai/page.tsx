...
export default function AIPage(){
 ...
 const context={signal:data?.signal,confidenceGate:data?.confidenceGate,invalidation:data?.invalidation,forecast:data?.forecast,scenarios:data?.scenarios,decisionAudit:data?.decisionAudit,risk:data?.risk,consensus:data?.consensus,multiTimeframe:data?.multiTimeframe,newsImpact:data?.newsImpact,news:data?.news?.slice(0,10),warnings:data?.warnings,analysts:data?.analysts?.slice(0,33)};
